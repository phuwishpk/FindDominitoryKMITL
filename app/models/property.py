...
# --- Association table ระหว่าง Property กับ Amenity ---
class PropertyAmenity(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("property_id", "amenity_id", name="uq_property_amenity"),
    )
...
class Amenity(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    code = db.Column(db.String(32), unique=True, nullable=False)  # เช่น 'ac','cctv','pet',...
...
class Property(db.Model):
    ...
    room_type = db.Column(db.String(30), nullable=False)             # 'studio','1br','2br','other'
    contact_phone = db.Column(db.String(20))
    line_id = db.Column(db.String(80))
    facebook_url = db.Column(db.String(255))

    # ราคา/ค่าบริการ
    rent_price = db.Column(db.Integer)                                # ค่าเช่า/เดือน
    ...
    # สถานะห้อง/เวิร์กโฟลว์
    availability_status = db.Column(db.String(16), default="vacant")  # 'vacant' หรือ 'occupied'
    workflow_status = db.Column(db.String(16), default="draft")       # 'draft','submitted','approved','rejected'
...
class PropertyImage(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    property_id = db.Column(db.Integer, db.ForeignKey("properties.id", ondelete="CASCADE"), nullable=False)
    file_path = db.Column(db.String(255), nullable=False)  # path ใต้ /uploads/...
